...
import { Bell, Droplet, Plus, Coffee, Wine, Milk, ChevronRight, Settings } from 'lucide-react';
...
// --- Modern "Micro-Shadow" Components ---

// 1. Stats Card (Clean, Vibrant Green)
const StatsSummaryCard = ({ label, value, unit, icon }: any) => (
  <div className="bg-white rounded-2xl p-4 flex flex-col items-center justify-center shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-50 min-w-[100px]">
    <div className="text-gray-400 mb-2">{icon}</div>
    <div className="text-xl font-bold text-gray-800">{value}<span className="text-xs text-gray-400 font-normal ml-1">{unit}</span></div>
    <div className="text-xs text-gray-500 font-medium">{label}</div>
  </div>
);

// 2. Drink Button (Horizontal Scroll Item)
const QuickDrinkBtn = ({ icon, label, amount, colorClass, onClick }: any) => (
  <button 
    onClick={onClick}
    className="flex flex-col items-center justify-center bg-white p-3 rounded-2xl shadow-[0_4px_16px_rgba(0,0,0,0.04)] active:scale-95 transition-all border border-transparent hover:border-gray-100 min-w-[80px]"
  >
...
const IconMap: any = {
    ...
    'wine': <Wine size={18} />,
    'milk': <Milk size={18} />,
    'beer': <Wine size={18} />, // Fallback
    'soda': <Wine size={18} />, // Fallback
};

export const HomeModern: React.FC<HomeProps> = ({ user, drinkOptions, onAddWater, onOpenSettings }) => {
  ...
  // Transition Logic (Kept from Original)
  ...
  useEffect(() => {
    ...
    if (stageSwitchTimerRef.current !== null) window.clearTimeout(stageSwitchTimerRef.current);
    stageSwitchTimerRef.current = window.setTimeout(() => {
      setCurrentStageIndex(targetStageIndex);

      if (transitionEndTimerRef.current !== null) window.clearTimeout(transitionEndTimerRef.current);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-[#fbffff] text-gray-800">
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative -mt-6">
        ...
        <div className="mb-8 flex flex-col items-center z-10">
             ...
             <div className="text-4xl font-black text-gray-800 flex items-baseline">
                {user.currentIntake}
                <span className="text-lg font-bold text-gray-400 ml-1">/ {user.dailyGoal}</span>
        ...
        <div className="relative w-[75vw] h-[75vw] max-w-[320px] max-h-[320px] flex items-center justify-center z-0">
             {/* Subtle Glow Behind Plant */}
            <div className={`absolute inset-0 bg-[#6fc172] rounded-full filter blur-[60px] opacity-10 transition-all duration-1000 ${isTransitioning ? 'scale-110 opacity-20' : 'scale-100'}`}></div>
      ...
      <div className="relative z-10 w-full bg-transparent pb-8 px-6 flex flex-col gap-6">
          ...
          {/* Motivation Text (Small & Clean) */}
          <div className="text-center">
             <p className="text-sm font-medium text-gray-400 italic">"每一滴水，都是森林成长的生命力。"</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
              <div className="bg-white p-4 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-50 flex items-center justify-between">
                  <div>
                      ...
                      <p className="text-xl font-bold text-gray-800">{user.streak} <span className="text-xs font-normal">天</span></p>
                  </div>
               ...
               <div className="bg-white p-4 rounded-2xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-gray-50 flex items-center justify-between">
                  <div>
                      ...
                      <p className="text-xl font-bold text-gray-800">{Math.round(progress)} <span className="text-xs font-normal">%</span></p>
                  </div>
          ...
          {/* Quick Drink Selector (Scrollable) */}
